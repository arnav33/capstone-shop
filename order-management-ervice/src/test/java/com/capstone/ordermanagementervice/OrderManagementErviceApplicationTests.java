...
import com.capstone.ordermanagementervice.controllers.OrderController;
...
class OrderControllerTest {
	...
	void testUpdateOrderStatus() {
		// Arrange
		OrderRequest orderRequest = new OrderRequest();
		doNothing().when(orderService).updateOrderStatus(orderRequest);

		// Act
		orderController.updateOrderStatus(orderRequest);

		// Assert
		verify(orderService, times(1)).updateOrderStatus(orderRequest);
	}
